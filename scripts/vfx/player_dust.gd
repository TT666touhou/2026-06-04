...
## 粒子壽命（秒）—— 越長軌跡越長
@export_range(0.2, 2.0, 0.01) var trail_lifetime: float = 0.55
## 粒子初速最小值（px/s）
...
## 粒子壽命（秒）
@export_range(0.2, 2.5, 0.01) var burst_lifetime: float = 0.75
## 爆發集中度（0=分散，1=同時）
@export_range(0.0, 1.0, 0.05) var burst_explosiveness: float = 0.88
## 粒子初速最小值（px/s）
@export_range(5.0, 400.0, 5.0) var burst_vel_min: float = 35.0
## 粒子初速最大值（px/s）
@export_range(5.0, 600.0, 5.0) var burst_vel_max: float = 95.0
## 重力（px/s²）
...
func _calc_burst_direction(normal: Vector2, vel: Vector2) -> Vector3:
	if vel.length_squared() > 0.01:
		var reflect := vel.bounce(normal).normalized()
		return Vector3(reflect.x, reflect.y, 0.0)
	# fallback：若速度為零，沿法線反方向
	var fallback := -normal.normalized()
	return Vector3(fallback.x, fallback.y, 0.0)
