...
## build_dungeon_palette_sources.gd
## ─────────────────────────────────────────────────────────────────
## 前置條件：
##   1. 已執行 scripts/tools/generate_dungeon_palette_pngs.py 生成 PNGs
##   2. 在 Godot FileSystem 右上角按 Rescan，等待 PNG 匯入完成
##   3. assets/tilesets/dungeonmode/dungeon_world_tileset.tres 已存在
##      (Source 0 = dungeon-mode.png，tile_size=8x8，物理碰撞已設定)
##
## 執行：Script 編輯器開啟此檔 → File → Run（Ctrl+Shift+X）
##
## 結果：dungeon_world_tileset.tres 重建所有彩色 Sources：
##   Source 0      = 原始黑白（保留物理碰撞設定）
##   Source 1~N    = 動態載入 colored/ 下的所有 PNG 圖源
## ─────────────────────────────────────────────────────────────────

const TILESET_PATH := "res://assets/tilesets/dungeonmode/dungeon_world_tileset.tres"
const COLORED_DIR  := "res://assets/tilesets/dungeonmode/colored/"

## 顏色名稱（與 generate_dungeon_palette_pngs.py 完全對應）
...
## 保留排序的原生圖源清單（透明背景系列優先，以確保 Source ID 穩定）
const PRESERVED_SOURCES: Array[String] = [
	# ── 城堡系列（與 mrmotext_world_tileset 完全一致）
...
func _run() -> void:
	print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
	print("  [Build Dungeon Palette Sources] 動態載入組合 PNG Sources")
	print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

	var tileset := load(TILESET_PATH) as TileSet
	if not tileset:
		printerr("❌ 找不到 TileSet：", TILESET_PATH)
		printerr("   請先在 Godot Editor 建立 dungeon_world_tileset.tres（Source 0 = dungeon-mode.png）")
		return

	if not tileset.has_source(0):
		printerr("❌ 沒有 Source 0，請先在 TileSet 編輯器加入 dungeon-mode.png 作為 Source 0")
		return
	...
	if not dir:
		...
		printerr("   請確認已執行 generate_dungeon_palette_pngs.py 並完成 Godot Rescan")
		return
	...
	# 排序：PRESERVED_SOURCES 的檔案優先保序，其餘字母排
	...
	for i in range(png_files.size()):
		...
		# 加入 TileSet（Source ID = i + 1）
		tileset.add_source(new_src, i + 1)

		# 複製 tile 座標與物理/導航/碰撞/遮擋/自訂數據
