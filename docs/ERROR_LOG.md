...
| 2026-06-22 | [ERR-GAP054] 盪繩無空中控制 | 右鍵盪繩時完全無法用方向鍵調整位置，靠重力盪繩太被動 | `_apply_movement` 在 `_wire != null` 時直接 return，不執行任何輸入 | 改為：計算繩子方向的切線向量（垂直於繩），左右鍵沿切線施加加速度（swing_accel=150 px/s²）；保留 return 以跳過一般水平移動邏輯 |
| 2026-06-22 | [ERR-GAP053] 盪繩速度分段 + 繩子視覺分段 | 右鍵盪繩時速度忽快忽慢（分段感），Line2D 12段 Verlet 繩子看起來一截一截 | (1) `velocity += correction/delta` 每幀注入大衝量與重力疊加造成 overshoot 循環；(2) Verlet 12段在鬆繩時重力下垂明顯，視覺分段；(3) 繩長 max_length 是 hard constraint，slack/taut 交替=速度突變 | (1) 廢除 velocity 衝量修正，改用 pre/post move_and_slide split：pre 只移除外向徑向速度，post 直接 global_position 賦值到繩長圓上（不注入速度）；(2) 廢除 Verlet rope，改用直線 Line2D（2 點），視覺零分段；(3) `snap_factor=0.12` 加微小彈力手感；(4) `_wire` 型別從 `RefCounted` 改為 `WireConstraint`（解決 GDScript parse error） |
| 2026-06-22 | [ERR-GAP052] Z方案手感不自然 | 右鍵盪繩時被拉近感覺不對，無法做出細緻動作 | GAP-051 Z方案固定徑向速度破壞了自然鐘擺弧；玩家感覺「被強制拉」而非「順物理擺盪」 | 廢除 Z 方案，改回純鐘擺：`constrain()` 只取消外向徑向速度（保留切向），`snap_factor=0.0`，位置修正轉 velocity（保留 move_and_slide），`swing_accel` 勾住期間停用（重力主導）。verify：max_len 平滑遞減至 min_length，dist 最終穩定 = 24 = max_len |
| 2026-06-22 | [ERR-GAP051] 收繩抖動 + 繩長不穩定 | auto_reel 縮短 max_length 後 constrain 產生 correction 衝量，與重力疊加造成每幀 overshoot，繩子視覺跳動 | GAP-048 改用 velocity 衝量後 correction 與重力/輸入疊加；位置不再保證在 max_length 圓上 | Z 方案：廢除 auto_reel + constrain，改為分解速度為切向+徑向，徑向固定設為 `auto_reel_speed`（300 px/s）朝錨點；視覺繩長改用實際距離。radial 恆定無抖動，平滑遞減至 min_length |
...
| 2026-06-24 | [ERR-GAP064] 即時勾爪 + 手動收縮設計決策 | 參考 Ronin：右鍵需即時固定最近牆面，無飛行時間；繩索不自動變短，只能手動收縮；Player 移動自動斷線 | 設計變更而非 bug | (1) `_start_grapple()` 改用 PhysicsRayQueryParameters2D raycast（mask=1, max=480px），命中即時 `place_wire_anchor_instant`，未命中靜默失敗；(2) 移除 `rope_reel_speed` 自動收繩，改為回合開始時執行 REEL_STEP=120px；(3) `_launch_slingshot/_shoot_attack` 先呼叫 `_release_grapple()`；(4) aim_preview 新增虛線範圍圈+命中/未命中指示器+收縮按鈕 |
| 2026-06-24 | [ERR-GAP063] Camera2D 作為 Player 子節點限制鏡頭移動 | 鏡頭無法在 FROZEN 階段自由平移/縮放觀察地形 | Camera2D 是 Player 的子節點，global_position 綁定 Player，無法在 FROZEN 時獨立移動 | 將 Camera2D 移至場景 root 層，新增 `camera_controller.gd`（`extends Camera2D`）：FROZEN 時 WASD 鍵盤平移 + 滾輪縮放 + 中鍵拖曳；PLAYING 時 lerp 跟隨 Player；同步擴展世界至 2560×1440 並配置 camera limits |
