...
| 2026-06-14 | [ERR-031] TileSet .tres 缺少 tile_size | 編輯器中 TileMapLayer 格線尺寸與遊戲像素座標不符（格線 16px vs 實際 8px tile） | TileSet `.tres` 的 `[resource]` 區塊**未顯式聲明 `tile_size`**，Godot 預設值為 `Vector2i(16,16)`。若 Atlas 的 texture_region_size 是 8×8，tile 格線 16px 比實際像素格線大 2 倍，Tilemap 編輯器中格線對不上。影響：無法精準對齊 Tile，Checkpoint 場景的 Appearance TileMapLayer 格線對不上房間像素格 | 在 `[resource]` 區塊明確加入 `tile_size = Vector2i(W, H)`（先確認圖片尺寸÷tile數量得到實際 tile 大小）。本專案：dungeon_world_tileset.tres 和 monochrome-transparent_packed.tres 均已修復。**Sensor v7 Check 12/12** 自動偵測所有 TileSet .tres 是否有顯式 tile_size |
---
