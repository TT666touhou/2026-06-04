...
| 2026-06-14 | [ERR-030] Variant 節點屬性 := 推斷失敗 | `Parser Error: Cannot infer the type of 'X' variable because the value doesn't have a set type.` | `get_node_or_null()` 在沒有型別標注時回傳 `Variant`。後續對該 Variant 的屬性（如 `.global_position`）用 `:=` 推斷，GDScript 無法確定型別，引發 Parser Error。典型案例：`var cam_zone = get_node_or_null("CameraZone")` → `var rect_center := cam_zone.global_position + ...` | **必須顯式型別標注**：`var cam_zone: Node2D = get_node_or_null("CameraZone") as Node2D`；接著 `var rect_center: Vector2 = cam_zone.global_position + ...`。或鏈式 cast：`var x: Vector2 = (get_node_or_null("X") as Node2D).global_position`。**Sensor v7 Check 11/12** 自動偵測此模式。❌ 不能用 `var cam_zone = get_node_or_null(...)` 後接 `:=` 存取屬性 |
| 2026-06-14 | [ERR-032] GDScript 同一 block 變數重複宣告 | `GDScript::reload: The variable "marker" is declared below in the parent block.` | 在同一個 `for` loop 中，inner scope 宣告 `var marker`（第 163 行），outer scope 同一迴圈再次宣告 `var marker`（第 169 行）。GDScript 在嚴格模式視同名變數為 shadow 警告。 | 改名 outer scope 的同名變數：`var root_marker: Marker2D = ...`，避免與 inner scope 衝突。Sensor TODO: 未來考慮掃描同 for-loop 下的 shadow 變數。 |
| 2026-06-14 | [ERR-033] 函式參數遮蔽基底類別屬性 | `GDScript::reload: The local function parameter "visible" is shadowing an already-declared property in the base class "CanvasItem".` | 繼承自 `CanvasItem`（透過 Area2D → Node2D）的腳本中，定義了 `func f(visible: bool)` — 參數名 `visible` 與 CanvasItem 的 `visible` 屬性同名。GDScript 嚴格模式報 shadowing 警告。 | 將函式參數改名：`func _set_hint_visible(show_hint: bool)`，函式體內用 `show_hint` 代替 `visible`。⚠️ 常見陷阱：`visible/position/rotation/scale/modulate/name` 都是 Node/CanvasItem 的屬性，**不能**作為函式參數名。 |
| 2026-06-14 | [ERR-034] Camera2D.make_current() 在節點進入場景樹之前呼叫 | `room_base.gd:208 @ _add_debug_camera(): Condition "!enabled \|\| !is_inside_tree()" is true.` | `cam.make_current()` 在 `player.add_child(cam)` 之前被呼叫。此時 cam 尚未加入任何 SceneTree，`is_inside_tree()` 為 false，make_current 靜默失敗。 | **嚴格順序**：必須先 `node.add_child(cam)` → 再 `cam.make_current()`。規則：任何依賴 `is_inside_tree()` 的 API（`make_current`, `set_as_top_level`, 信號連接等）必須在加入場景樹後才呼叫。 |
| 2026-06-14 | [ERR-035] SpawnPoint 無效警告（有 Checkpoint 時仍警告）| `room_base.gd:27 @ _ready(): [RoomBase] No SpawnPoint found in room: area0_room_01` | `_ready()` 中無論是否有 Checkpoint 都強制警告 SpawnPoint 缺失。但設計上有 Checkpoint 的房間根本不需要 SpawnPoint（spawn 優先使用 Checkpoint.get_spawn_position()）。 | 移除 `_ready()` 中的 SpawnPoint 強制警告。邏輯移至 `_get_debug_spawn_info()` 內的 fallback 分支 — 只有在既無 Checkpoint、又無 Portal SpawnMarker 時才 fallback 到 `Vector2(80, -80)` 不做任何警告（fallback 行為已足夠）。 |
---
