...
【第三步（GAP-011 強制）：GDD 編輯紀律三項檢查 — 寫入前必做】
□ 6. 衝突偵測：新內容是否與任何現有 [CONFIRMED] 條目矛盾？
      - 若矛盾：先 grill-me 釐清，再決定是更新舊內容還是推翻新設計
      - 不得「兩者並存」製造歧義
□ 7. 過時內容偵測：GDD 是否有與現行 codebase 不符的描述？
      - 對照最近的 git log（git log --oneline -10 D:\2026-06-04）
      - 發現過時內容必須同步修正，不得留存舊描述
□ 8. 結構紀律：新內容必須整合進對應章節，不得僅追加至文件末段
      - 散落的更新紀錄 → 移至附錄 B「更新記錄」
      - 散落的操控表 → 整合至 §6.2 或對應章節
      - 禁止出現兩個相同章節編號（§10、§10.11 重複等）
