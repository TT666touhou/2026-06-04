...
## 【GLOBAL-RULE-005】禁止超出用戶要求範圍（2026-06-24 硬性規則）

> [!CAUTION]
> **實作任何功能時，只做用戶明確要求的事，不得自行添加「感覺合理」的額外修改。**

### 規則細節

| 情境 | 強制做法 |
|------|---------|
| 用戶要求加入某功能 | 只加該功能；不改場景佈局、不加平台、不改世界大小 |
| 需要更多空間/地形配合新功能 | **先問用戶**，不得自行判斷並修改 |
| 用戶要求「斷線」條件 | 嚴格按用戶說的條件實作，不擴大到其他動作 |
| 任何 .tscn 場景節點的增減 | 必須有用戶明確的對應要求，否則禁止修改 |

### 典型違規模式（GAP-063/064 教訓）

- ❌ 用戶要求「加鏡頭」→ 自行把世界從 1280×720 擴大到 2560×1440 並加 3 個平台
- ❌ 用戶說「Player 移動時斷線」→ 自行擴大到「攻擊針也斷線」
- ❌ 用戶沒提 GDD 更新 → 跳過 GAME_DESIGN.md 更新

### 強制檢查（commit 前自問）

1. 我改的每一行，用戶有沒有明確要求這個？
2. 有沒有任何「感覺應該順便加的」內容？如果有 → 刪掉或先問。
